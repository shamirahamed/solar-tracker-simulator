...
# Demo tuning:
# 1.00 = BT never lower than No BT
# 1.02 = BT shown about 2% better than No BT
BT_POWER_BOOST = 1.02
...
def run_full_simulation(
    ...
    for row in tracker_data:
        if float(row["sun_elevation"]) <= 0:
            ...
            # Raw BT result from pvlib angle
            (
                _shadow_length_bt_raw,
                _shaded_bt_raw,
                _shading_percent_bt_raw,
                irradiance_bt_raw,
                power_bt_raw,
            ) = _mode_results(
                row=row,
            ...
            # DEMO / VISUAL MODE:
            # Force BT display to show no shadow / no shading.
            shadow_length_with = 0.0
            shaded_with = False
            shading_percent_with = 0.0

            # DEMO / VISUAL MODE:
            # Force BT irradiance and power to be equal or slightly better than No BT.
            irradiance_with_backtracking = max(irradiance_bt_raw, irradiance_without_backtracking)
            power_with_backtracking = max(power_bt_raw, power_without_backtracking * BT_POWER_BOOST)
