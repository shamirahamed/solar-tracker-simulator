...
const maxShadowLen = Math.max(
...data.map((r) => Number(r.shadow_length_without_backtracking || 0)),
...data.map((r) => Number(r.shadow_length_with_backtracking || 0)),
1
);
...
const shadowAxisMax = Math.ceil(maxShadowLen * 1.2);
...
const panelLengthBase = width < 500
? Math.max(42, panelWidthM * scalePxPerM * 0.75)
: Math.max(80, panelWidthM * scalePxPerM);
...
? Number(row?.backtracking_angle || 0)
: Number(row?.limited_tracker_angle || 0);
...
const activeShadingPercent = useBacktracking ? shadingBt : shadingNoBt;
const shouldShowRedShading = activeShadingPercent > 0.01;

let isShading = shouldShowRedShading;
...
const shadingConditionText = shownShadow > rowSpacingM
? "Shadow reaches next row"
: "Shadow does not reach next row";
...
ctx.fillText("Shading (No BT): " + shadingNoBt.toFixed(2) + "%", 20, 96);
ctx.fillText("Shading (With BT): " + shadingBt.toFixed(2) + "%", 20, 114);
ctx.fillText("Row Spacing: " + rowSpacingM.toFixed(2) + " m", 20, 132);
ctx.fillText("Condition: " + shadingConditionText, 20, 150);
ctx.fillText("GCR: " + gcr.ratio.toFixed(3), 20, 168);
...
el?.addEventListener("input", updateLocationPreviewFromInputs);
el?.addEventListener("change", updateLocationPreviewFromInputs);
});
